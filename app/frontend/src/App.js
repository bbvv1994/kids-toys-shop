...
function CMSProducts({ mode, editModalOpen, setEditModalOpen, editingProduct, setEditingProduct, dbCategories }) {
  ...
  const handleDelete = async (id) => {
    ...
    console.log('🗑️ CMS: Requesting confirmation for deletion');
    if (!window.confirm('Вы уверены, что хотите удалить этот товар? Это действие нельзя отменить.')) {
      console.log('🗑️ CMS: Deletion cancelled by user');
      return;
    }
    ...
    try {
      ...
      if (!checkResponse.ok) {
        alert('Товар не найден или у вас нет прав для его удаления.');
        return;
      }
  ...
  const handleToggleHidden = async (product) => {
    try {
      ...
      if (response.ok) {
        alert(`Товар успешно ${newHiddenValue ? 'скрыт' : 'показан'}.`);
        fetchProducts();
        // Обновляем все состояния товаров в приложении
        if (window.refreshAllProducts) {
          window.refreshAllProducts();
        ...
        alert(`Ошибка при изменении видимости товара: ${errorMessage}`);
      }
    } catch (error) {
      console.error('Error toggling product visibility:', error);
      alert('Ошибка при изменении видимости товара. Проверьте подключение к серверу.');
    }
  };
  ...
  const handleEditSubmit = async e => {
    ...
    try {
      ...
      setCategories(prevCategories => {
        ...
        console.log('🔄 Обновленное состояние категорий:', updatedCategories.find(c => c.id === categoryId));
      ...
      // Обновляем боковое меню асинхронно с принудительным обходом кэширования
      if (loadCategoriesFromAPI) {
        setTimeout(() => {
          console.log('🔄 Обновляем боковое меню после редактирования категории');
          loadCategoriesFromAPI(true); // Принудительное обновление
        }, 200);
      }
