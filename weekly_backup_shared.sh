...
# 🗓️ Еженедельный бэкап на Google Drive через Service Account в Shared Drive
...
# 5. Загрузка на Google Drive через Service Account в Shared Drive
log "☁️ Загрузка на Google Drive (Shared Drive)..."
cd $GOOGLE_DRIVE_DIR
...
python3 service_account_upload_shared.py "$FULL_BACKUP_FILE" "kids-toys-full-backup-$WEEK.tar.gz"

# Загружаем отдельные файлы
python3 service_account_upload_shared.py "$DB_BACKUP_FILE" "db_backup_$DATE.sql"
python3 service_account_upload_shared.py "$UPLOADS_BACKUP_FILE" "uploads_backup_$DATE.tar.gz"
python3 service_account_upload_shared.py "$CONFIG_BACKUP_FILE" "config_backup_$DATE.tar.gz"
...
echo "   - Всего загружено в Shared Drive: 4 файла"
