...
export default function BoysToysPage({ 
  products, 
  onAddToCart, 
  cart, 
  ...
  const isMobile = deviceType === 'mobile';
   ...
   const boysProducts = useMemo(() => {
     ...
     const filtered = products.filter(product => {
       ...
       // Проверяем, что пол указан как "для мальчиков"
       const isForBoys = product.gender && typeof product.gender === 'string' && (
         product.gender === 'Для мальчиков' ||
         product.gender === 'Мальчики' || 
         product.gender === 'boys' ||
         product.gender === 'Boys' ||
         product.gender.toLowerCase().includes('мальчик') ||
         product.gender.toLowerCase().includes('boy')
       );
       
       // Возвращаем true только если ОБА условия выполнены
       return isToyCategory && isForBoys;
     });
      ...
      <Container maxWidth="lg" sx={{ py: 4, minHeight: '80vh' }}>
        <Box sx={{ 
          ...
          <Typography variant="h6" sx={{ color: '#666' }}>
            {t('catalog.pages.boysToys.loading')}
          </Typography>
        </Box>
      </Container>
    );
  ...
  if (!isLoading && !boysProducts.length) {
    return (
      <Container maxWidth="lg" sx={{ py: 4, minHeight: '80vh' }}>
        <Box sx={{ textAlign: 'center', py: 8 }}>
          <Typography variant="h4" sx={{ color: '#666', mb: 2 }}>
            {t('catalog.pages.boysToys.title')}
          </Typography>
          <Typography variant="h6" sx={{ color: '#888', mb: 4 }}>
            {t('catalog.pages.boysToys.noProducts')}
          </Typography>
          <Typography variant="body1" sx={{ color: '#999', maxWidth: 500, mx: 'auto' }}>
            {t('catalog.pages.boysToys.noProductsDescription')}
          </Typography>
          <Button 
            onClick={() => navigate('/')}
            variant="contained" 
          ...
          >
            {t('catalog.pages.boysToys.backToHome')}
          </Button>
        </Box>
      </Container>
    );
