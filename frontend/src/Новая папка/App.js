...
function App() {
  ...
  // Отладочная информация для модального окна редактирования
  useEffect(() => {
    console.log('editModalOpen изменился на:', editModalOpen);
  }, [editModalOpen]);
  
  useEffect(() => {
    console.log('editingProduct изменился на:', editingProduct);
  }, [editingProduct]);
...
function AppContent({ 
  ...
  return (
    <>
      ...
      />
      {/* Отладочная информация для EditProductModal */}
      {console.log('=== RENDER EditProductModal ===')}
...
function CMSProducts({ mode, editModalOpen, setEditModalOpen, editingProduct, setEditingProduct, dbCategories }) {
  console.log('=== CMSProducts RENDER ===');
  ...
  const [openSelects, setOpenSelects] = React.useState({
    category: false,
    subcategory: false,
    ageGroup: false,
    gender: false
  ...
  const handleOpenEdit = (product) => {
    console.log('=== handleOpenEdit ВЫЗВАН ===');
    console.log('Продукт:', product);
    console.log('Текущее состояние editModalOpen:', editModalOpen);
    console.log('Текущее состояние editingProduct:', editingProduct);
    ...
    console.log('Продукт с callback функциями:', productWithCallbacks);
    console.log('Устанавливаем editingProduct...');
    setEditingProduct(productWithCallbacks);
    console.log('Устанавливаем editModalOpen в true...');
    setEditModalOpen(true);
    ...
    console.log('=== handleOpenEdit ЗАВЕРШЕН ===');
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (!user || !user.token) {
      console.log('ОШИБКА: Нет токена авторизации');
      alert('Ошибка авторизации. Пожалуйста, войдите в систему.');
      return;
    }
    
    console.log('Токен найден, продолжаем удаление...');
  ...
  const handleToggleHidden = async (product) => {
    console.log('=== handleToggleHidden ВЫЗВАН ===');
    console.log('Продукт для изменения видимости:', product);
    console.log('Текущее состояние isHidden:', product.isHidden);
    ...
    try {
      ...
      console.log('Отправка запроса на изменение видимости...');
      console.log('URL:', `${API_BASE_URL}/api/products/${product.id}/hidden`);
      console.log('Новое состояние isHidden:', !product.isHidden);
      ...
      if (response.ok) {
        console.log('Видимость успешно изменена, обновляем список товаров...');
        fetchProducts();
      } else {
        const errorData = await response.json();
        console.error('Ошибка сервера:', errorData);
      ...
      console.error('=== ОШИБКА handleToggleHidden ===');
      console.error('Error toggling product visibility:', error);
      alert('Ошибка при изменении видимости товара');
    }
    
    console.log('=== handleToggleHidden ЗАВЕРШЕН ===');
  };
          ...
          <tbody>
            {filteredProducts.map(p => (
              <tr key={p.id}>
                ...
                <td style={{ padding: 8, border: '1px solid #eee', whiteSpace: 'nowrap', position: 'relative', zIndex: 9999 }}>
                  {console.log('Рендерится ячейка с кнопками для товара:', p.id, p.name)}
                  <Box 
                    sx={{ display: 'flex', gap: 0.5, justifyContent: 'center', alignItems: 'center', position: 'relative', zIndex: 10000 }}
                    onClick={(e) => {
                  ...
                  >
                      <IconButton 
                        ...
                        onClick={(e) => {
                          ...
                          console.log('Название товара:', p.name);
                          console.log('handleOpenEdit функция:', typeof handleOpenEdit);
                          console.log('Вызываем handleOpenEdit...');
                          try {
                            handleOpenEdit(p);
                            console.log('handleOpenEdit вызван успешно');
                          } catch (error) {
                            console.error('Ошибка при вызове handleOpenEdit:', error);
                          }
                        }}
                        onMouseDown={(e) => {
                          console.log('=== MOUSEDOWN ПО КНОПКЕ РЕДАКТИРОВАНИЯ ===');
                          console.log('MouseDown событие:', e);
                        }}
                        onMouseUp={(e) => {
                          console.log('=== MOUSEUP ПО КНОПКЕ РЕДАКТИРОВАНИЯ ===');
                          console.log('MouseUp событие:', e);
                        }}
                        sx={{ 
                          color: '#1976d2',
                          '&:hover': {
                      ...
                      {console.log('Рендерится кнопка редактирования для товара:', p.id, p.name)}
                      <IconButton 
                        size="small" 
                        onClick={() => {
                      ...
                      {console.log('Рендерится кнопка удаления для товара:', p.id, p.name)}
                      <IconButton 
                        size="small" 
                        onClick={() => {
                          console.log('Клик по кнопке скрытия/показа для товара:', p);
                          console.log('Текущее состояние isHidden:', p.isHidden);
                      ...
                      {console.log('Рендерится кнопка скрытия для товара:', p.id, p.name, 'isHidden:', p.isHidden)}
                  </Box>
                </td>
              </tr>
            ))}
