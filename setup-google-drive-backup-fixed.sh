...
if [ "$EUID" -ne 0 ]; then
    error "Пожалуйста, запустите скрипт от root: sudo bash setup-google-drive-backup-fixed.sh"
fi
...
# Установка Google Drive API (с проверкой)
log "Проверка установки Google Drive API библиотек..."
if python3 -c "import google.auth" 2>/dev/null; then
    log "✅ Google Drive API библиотеки уже установлены"
else
    log "Установка Google Drive API библиотек..."
