...
log "☁️ Загрузка на Google Drive (Service Account)..."
cd $GOOGLE_DRIVE_DIR
...
python3 service_account_upload_personal.py "$FULL_BACKUP_FILE" "kids-toys-full-backup-$WEEK.tar.gz"

# Загружаем отдельные файлы
python3 service_account_upload_personal.py "$DB_BACKUP_FILE" "db_backup_$DATE.sql"
python3 service_account_upload_personal.py "$UPLOADS_BACKUP_FILE" "uploads_backup_$DATE.tar.gz"
python3 service_account_upload_personal.py "$CONFIG_BACKUP_FILE" "config_backup_$DATE.tar.gz"
