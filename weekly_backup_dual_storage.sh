...
# Включает базу данных (с Cloudinary URL), локальные изображения (включая резервные) и конфигурацию
...
log "🗓️ Начало улучшенного еженедельного бэкапа с двойным хранением"
log "📅 Дата: $(date)"
log "📊 Неделя: $WEEK"
log "☁️ Поддержка: Cloudinary + Локальные резервные копии"
echo "=================================================="
        ...
        if 'imageUrls' in product and product['imageUrls']:
            for url in product['imageUrls']:
                if url.startswith('http') and 'cloudinary.com' in url:
                    ...
                    local_count += 1
                # Проверяем, есть ли информация о двойном хранении в storageInfo
                if hasattr(product, 'storageInfo') and product['storageInfo']:
                    for storage in product['storageInfo']:
                        if storage.get('primary') == 'cloudinary' and storage.get('fallback') == 'local':
                            dual_storage_count += 1
    
    print(f'{{ \"cloudinary\": {cloudinary_count}, \"local\": {local_count}, \"dual_storage\": {dual_storage_count} }}')
...
if echo "$API_STATS" | grep -q "error"; then
    ...
    DUAL_STORAGE_COUNT=$(echo "$API_STATS" | jq -r '.dual_storage')
    log "📊 Статистика изображений в БД (через API):"
    log "   - Cloudinary URL: $CLOUDINARY_COUNT"
    log "   - Локальные пути: $LOCAL_COUNT"
    log "   - Двойное хранение: $DUAL_STORAGE_COUNT"
fi

# 3. Бэкап локальных изображений (включая резервные)
info "🖼️ Создание бэкапа локальных изображений (включая резервные)..."
...
log "✅ Улучшенный еженедельный бэкап с двойным хранением завершен!"
log "📊 Статистика:"
...
echo "   - Двойное хранение: $DUAL_STORAGE_COUNT"
...
log "   - Локальные резервные копии восстановятся из uploads backup"
log "   - Система автоматически выберет лучшее доступное хранилище"
echo "=================================================="
