#!/bin/bash
# Еженедельный бэкап на Google Drive
...
python3 upload_to_drive.py "$BACKUP_DIR/full_backup_$WEEK.tar.gz" "kids-toys-full-backup-$WEEK.tar.gz"

# Загружаем отдельные файлы
python3 upload_to_drive.py "$BACKUP_DIR/db_backup_$DATE.sql" "db_backup_$DATE.sql"
python3 upload_to_drive.py "$BACKUP_DIR/uploads_backup_$DATE.tar.gz" "uploads_backup_$DATE.tar.gz"
python3 upload_to_drive.py "$BACKUP_DIR/config_backup_$DATE.tar.gz" "config_backup_$DATE.tar.gz"
